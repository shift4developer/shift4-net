...
namespace Shift4Tests.Integration
{
    public class PayoutTests : IntegrationTest
    {
        ...
        public async Task RetrievePayout()
        {
            ...
            var listedPayout = (await _gateway.ListPayouts()).List.First(p => p.Id == payout.Id);
        ...
        public async Task RetrievePayoutTransactions()
        {
            ...
            Assert.Equal(1, chargePayoutTransaction.List.Count);
            ...
            Assert.Equal(chargeTransaction.Source.Id, charge.Id);
            Assert.True(chargeTransaction.Fee > 0);
            Assert.NotNull(chargeTransaction.Id);
        }

        [Fact]
        public async Task RetrievePayoutTransactionsWithExpandedSource(){
            // given
            var payout = await _gateway.CreatePayout();
            var charge = await _gateway.CreateCharge(new ChargeRequest()
                {
            ...
            payout = await _gateway.CreatePayout();
            //when
            var request = new PayoutTransactionListRequest() { Source = charge.Id }
                .expandSource();
            var chargePayoutTransaction = await _gateway.ListPayoutTransactions(request);

            //then
            Assert.NotNull(chargePayoutTransaction);
            Assert.Equal(1, chargePayoutTransaction.List.Count);
            var chargeTransaction = chargePayoutTransaction.List[0];
            Assert.True(chargeTransaction.Source.Expanded);
            Assert.True(chargeTransaction.Source.ExpandedObject is Charge);
            Assert.Equal(chargeTransaction.Source.Id, charge.Id);
            var sourceCharge = (Charge)chargeTransaction.Source.ExpandedObject;
            Assert.Equal(sourceCharge.Id, charge.Id);
        }
    }
}
